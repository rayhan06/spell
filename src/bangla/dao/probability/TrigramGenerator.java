...
class TrigramDTO
{
	...
	Long firstWordID, secondWordID, thirdWordID;
	int frequency;
}
...
public class TrigramGenerator 
{
	...
	{
		System.out.println("Starting TrigramGenerator ");
		...
		HashMap<String,TrigramDTO> trigramMap = new HashMap<String,TrigramDTO>(200000000);
...
//					List <String>wordTokenList =  WordTokenizer.tokenize(content);
...
//					char wordNormalizedOutput[] = new char[512];
						...
						else if( (ch<0x0980||ch>0x09E3) && (ch<0x200A||ch>0x200E) )
						{
							...
							else
							{
								if(i-startIndex>1)
								{
									...
									if (wordLength>0)
									{
										thirdWord = new String(contentChar, startIndex, wordLength);
										trigramCount = processWord(firstWord, secondWord, thirdWord, trigramMap,trigramCount,wordIDMap);			
					...
					{						
						...
						{
							thirdWord = new String(contentChar, startIndex, i-startIndex+lastCharLen);
							trigramCount = processWord(firstWord, secondWord, thirdWord, trigramMap,trigramCount, wordIDMap);			
							firstWord = secondWord;
							secondWord = thirdWord;
						}
			...
			rs=null;
				if(sqli+1==sql.length/2|| sqli==sql.length-1)
				{
					...
					for(Entry<String, TrigramDTO> entry:trigramMap.entrySet())
					{
						String key = entry.getKey();
						TrigramDTO dto = entry.getValue();
						if(dto.frequency<=1)
						{
							removeList.add(key);
						}
					}
					System.out.println("Removing "+removeList.size()+" Trigrams ");
			
					for(String key:removeList)
					{
						trigramMap.remove(key);
					}
					removeList.clear();
					removeList=null;
		...
		System.out.println("All Trigram generation complete. Total:"+trigramCount);
		...
		try
		{
			String insertSql = "insert into word_trigram values (?,?,?,?,?)";
		
			connection = DBMW.getInstance().getConnection();
			ps = connection.prepareStatement(insertSql);
			System.out.println("Starting Trigram insertion...");
			int count=0;
			for(Entry<String, TrigramDTO> entry:trigramMap.entrySet())
			{
				TrigramDTO dto = entry.getValue();
				if(dto.frequency<=4)continue;
				ps.setLong(1, DBMW.getInstance().getNextSequenceId("word_trigram"));
				ps.setInt(2, dto.firstWordID.intValue());
				ps.setInt(3, dto.secondWordID.intValue());
				ps.setInt(4, dto.thirdWordID.intValue());
				ps.setInt(5, dto.frequency);
			...
			System.out.println("All Trigram insertion complete.");
			System.out.println("Time taken:"+(System.currentTimeMillis()- startTime )+" milliseconds");
			Thread.sleep(100);
			System.exit(0);
		}
	...
	public static long  processWord(String firstWord, String secondWord, String thirdWord, HashMap<String,TrigramDTO> trigramMap, long trigramCount,
			HashMap<String,Long>wordIDMap )
	{
		
		
//		for(String thirdWord:wordTokenList)
		{
			if(firstWord!=null)
			{
				...
				keyBuf.append(';');
				keyBuf.append(thirdWord);
				
				String key = keyBuf.toString();
				TrigramDTO dto = trigramMap.get(key);
				if(dto==null)
				{
					dto = new TrigramDTO();
					...
					if(dto.firstWordID!=null)
					{
						...
						{
							dto.thirdWord=thirdWord;
							dto.thirdWordID = wordIDMap.get(thirdWord);
							if(dto.thirdWordID!=null)
							{
								dto.frequency=1;
								trigramMap.put(key, dto);
								trigramCount++;
								if(trigramCount%100000==0)
									System.out.println("Trigram Generation:"+(trigramCount/1000000.0) +" million");
