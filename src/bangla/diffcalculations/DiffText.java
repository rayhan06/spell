package bangla.diffcalculations;
...
public class DiffText 
{
	...
	public static void main(String[] args) 
	{
		...
		String originalContent = "আমাদের দেশ, মাটি,         নদীপথ, পাখি, সবই, আমি ভালবাসি?";
		String correctedContent = "আমাদের দেশ, মাটি, নদীকথ, পাখি সবই আমি ভালবাসি।";
...
//				"";
		...
		List<Map<Object, Object>> result = dt.alignText(originalContent, correctedContent);

		int itemNo = 1;
		for(Map<Object, Object> listItem: result)
		{
			System.out.println("Item: " + itemNo++);
			
			if(!listItem.get("original").equals(listItem.get("corrected")))
			{
				for(Object key:listItem.keySet())
				{
					Object value = listItem.get(key);
	...
	public List<WordDTO> getDifferentDtos(String originalContent, String correctedContent, int startIndex)
	{
		List<WordDTO> result = new ArrayList<WordDTO>();
		
		DiffText dt = new DiffText();
		List<Map<Object, Object>> alignment = dt.alignText(originalContent, correctedContent);

		for(Map<Object, Object> listItem: alignment)
		{
			if(!((String)listItem.get("original")).equalsIgnoreCase(((String)listItem.get("corrected"))))
			{
				WordDTO newDto = new WordDTO();
				newDto.offset = (int)listItem.get("startIndex");
				newDto.word = (String)listItem.get("original");
//				newDto.wordSuggestionList = new ArrayList<Pair>();
				newDto.length = newDto.word.length();
				WordSuggestionDTO p = new WordSuggestionDTO();
				p.suggestionClass = 50;
				p.suggestionWord = (String)listItem.get("corrected");
				newDto.addSuggestion(p);
				
				if(newDto.offset == -1)newDto.offset = startIndex + newDto.offset + originalContent.length();
				else newDto.offset += startIndex;
	...
	public List<Map<Object, Object>> alignText(String originalContent, String correctedContent)
	{

		diff_match_patch dmp = new diff_match_patch();
		
		LinkedList<diff_match_patch.Diff> diff = dmp.diff_main(originalContent, correctedContent);
		
		dmp.diff_cleanupSemantic(diff);
		List<Map<Object, Object>> alignment1 = findWordIndexV2(diff, originalContent);
		List<Map<Object, Object>> alignment2 = mergeL1(alignment1);
		List<Map<Object, Object>> alignment3 = mergeL2(alignment2);
		
		return alignment3;
		
	}
	
	public List<Map<Object, Object>> findWordIndexV2(LinkedList<diff_match_patch.Diff> diff, String originalContent)
	{
		int wi = 0;
		
		List<Map<Object, Object>> alignment = new LinkedList<>();
...
//					map.put("index", wi);
	...
	public List<Map<Object, Object>> mergeL1(List<Map<Object, Object>> align)
	{
		List<Map<Object, Object>> alignment = new LinkedList<>(); 
		...
		for(Map<Object, Object> map : align)	
		{
...
//			token.put("index", index);
	...
	public List<Map<Object, Object>> mergeL2(List<Map<Object, Object>> alignment)
	{
		ListIterator<Map<Object, Object>> iter = alignment.listIterator();
...
//					elem.put("index", (int)top.get("index"));
		...
		Stack<Map<Object, Object>> stackr = new Stack<>();
		
		while(!stack.isEmpty())
		{
			Map<Object, Object> top = stack.pop();
			
			stackr.push(top);
		}
		
		List<Map<Object, Object>> target = new LinkedList<>();
